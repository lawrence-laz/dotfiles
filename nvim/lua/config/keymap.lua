...
-- <C-f> command mode with vim like buffer (while cmd line is active ":")
-- <C-w> + - expands, s - horizontal split, v - vertical split
-- TODO: install and start using https://github.com/casey/just? or just simple Run.sh and Test.sh scripts are good enough?
-- :%s/before/after/g         - /g makes it global?; for new lines use \r
-- :s/before/after/           - current line only, /g applies multiple times, default only first match
-- :read !ls		      - Command output to current buffer
...
if vim.g.vscode then
    ...
    vim.keymap.set("i", "<C-k>", function() vim.lsp.buf.signature_help() end) -- show errors under cursor
